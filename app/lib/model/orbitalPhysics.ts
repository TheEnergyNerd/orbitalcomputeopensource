...
export function computeThermalSystemCost(
  ...
  // INVARIANT CHECK: If thermalCapFactor implies 32 kW rejectable, the reported area must be consistent within 1%
  const expectedAreaFromCap = conservativeWasteHeatKw * thermalCapFactor * 1000 / effectiveFluxWPerM2;
  const areaError = Math.abs(areaAvailableM2 - expectedAreaFromCap) / areaAvailableM2;
  if (areaError > 0.01) {
    throw new Error(
