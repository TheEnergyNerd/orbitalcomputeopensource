  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-6 pt-24 sm:pt-28">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Error Banner */}
        {Array.isArray(debugState.errors) && debugState.errors.length > 0 && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Ceiling Stack Chart */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="ceiling-stack"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <CeilingStackChart debugState={debugState} />
          </div>
          
          {/* Failure vs Replacement Chart */}
          <div className="rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="failure-replacement"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <FailureReplacementChart debugState={debugState} />
          </div>
          
          {/* Thermal Utilization Gauge */}
          <div className="rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <ThermalUtilizationGauge currentState={currentState} />
          </div>
          
          {/* Launch Economics Slider */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <LaunchEconomicsSlider />
          </div>
          
          {/* Autonomy Maturity Curve */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="autonomy-maturity"> {/* CRITICAL: Expanded panel vertically with more padding */}
          ...
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="energy-return"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <EnergyReturnOnLaunch debugState={debugState} />
          </div>
          
          {/* Power Stranded vs Power Used */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="power-stranded"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <PowerStrandedChart debugState={debugState} />
          </div>
          
          {/* Thermal Rejection Margin */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="thermal-rejection"> {/* CRITICAL: Expanded panel vertically with more padding */}
            <ThermalRejectionMargin debugState={debugState} />
          </div>
          
          {/* Radiator Scaling vs Compute Density */}
          <div className="lg:col-span-2 rounded-2xl border border-slate-800 bg-slate-950/85 px-3 sm:px-4 py-4" data-chart="radiator-scaling"> {/* CRITICAL: Expanded panel vertically with more padding */}
