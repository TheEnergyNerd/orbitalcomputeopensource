...
import type { OrbitalPowerPoint } from "../../lib/orbitSim/selectors/physics";
import type { ScenarioMode } from "../../lib/orbitSim/simulationConfig";

interface OrbitalPowerChartProps {
  data: OrbitalPowerPoint[];
  currentYear?: number;
  scenarioMode?: ScenarioMode;
}

/**
 * Orbital Power (GW) Chart
 * Shows total orbital power scaling to 150 GW by 2040
 */
export default function OrbitalPowerChart({
  data,
  currentYear,
  scenarioMode,
}: OrbitalPowerChartProps) {
  ...
  useEffect(() => {
    ...
    const maxPower = d3.max(data, d => d.powerGW) ?? 1;
    ...
    const yAxis = d3.axisLeft(yScale)
      .tickFormat(d => {
        const val = Number(d);
        if (val >= 1) return `${val.toFixed(0)} GW`;
        return `${(val * 1000).toFixed(0)} MW`;
      });
    ...
    g.append("text")
      ...
      .style("font-size", isMobile ? "10px" : "12px")
      .style("fill", "#94a3b8")
      .text("Orbital Power (GW)");

    // Add 150 GW target line
    g.append("line")
      .attr("x1", 0)
      .attr("x2", innerWidth)
      .attr("y1", yScale(150))
    ...
    // Area chart
    const area = d3.area<OrbitalPowerPoint>()
      .x(d => xScale(d.year))
      .y0(innerHeight)
      .y1(d => yScale(d.powerGW))
        ...
        tooltip
          ...
          .html(`
            <div style="background: rgba(15, 23, 42, 0.95); padding: 8px; border-radius: 4px; border: 1px solid rgba(148, 163, 184, 0.2);">
              ...
              <div style="color: #3b82f6; font-size: 11px;">Orbital Power: ${closest.powerGW.toFixed(2)} GW</div>
            </div>
          `);
      };
