...
import type { LaunchMassPoint } from "../../lib/orbitSim/selectors/constraints";
import type { ScenarioMode } from "../../lib/orbitSim/simulationConfig";

interface LaunchMassChartProps {
  data: LaunchMassPoint[];
  currentYear?: number;
...
/**
 * Launch Mass Chart
 * Shows launchMassThisYearKg vs launchMassCeiling
 */
export default function LaunchMassChart({
  data,
  currentYear,
  scenarioMode,
}: LaunchMassChartProps) {
  ...
  useEffect(() => {
    ...
    // Set up scales
    const maxMass = d3.max(data, d => Math.max(d.usedKg, d.ceilingKg)) ?? 1;
    ...
    const yAxis = d3.axisLeft(yScale)
      .tickFormat(d => `${(Number(d) / 1000).toFixed(0)}k`); // Just numbers, unit is in axis label
    ...
    const ceilingLine = d3.line<LaunchMassPoint>()
      .x(d => xScale(d.year))
      .y(d => yScale(d.ceilingKg))
      .curve(d3.curveMonotoneX);
    ...
    g.append("path")
      .datum(data)
      .attr("fill", "none")
      .attr("stroke", "#3b82f6") // blue for used
      .attr("stroke-width", 2)
      .attr("d", usedLine);
    ...
    g.append("path")
      .datum(data)
      .attr("fill", "none")
      .attr("stroke", "#ef4444") // red for ceiling
      .attr("stroke-width", 2)
      ...
      .on("mousemove", function(event) {
        ...
        if (tooltipRef.current && point) {
          ...
          tooltipRef.current.innerHTML = `
            ...
            <div style="color: #3b82f6">Used: ${(point.usedKg / 1000).toFixed(1)}k kg</div>
            <div style="color: #ef4444">Ceiling: ${(point.ceilingKg / 1000).toFixed(1)}k kg</div>
            <div>Headroom: ${((point.ceilingKg - point.usedKg) / 1000).toFixed(1)}k kg</div>
          `;
        }
      })
      .on("mouseout", () => {
