...
export interface ModelAnalysis {
  crossover: {
    year: number | null;
    crossoverYearGpuHourStandard?: number | null; // Crossover using GPU-hour pricing
    crossoverYearEffectivePflop?: number | null; // Crossover using effective PFLOP-year cost
    orbitalPrice: number;
    groundPrice: number;
...
export interface YearlyBreakdown {
  ...
  ground: {
    ...
    constraints?: {
      ...
      delayPenalty: number;
      scarcityRentPerPflopYear?: number; // Saturating wait-time-based rent (Hill function)
      appliedMultipliers: {
        constraintMultiplierUsed: boolean;
        energyMultiplierUsed: boolean;
        siteMultiplierUsed: boolean;
      };
      waccBase?: number; // Base WACC
      waccEffective?: number; // Effective WACC (rises with backlog)
      rentFrac?: number; // Rent fraction (0..1) from Hill function
      waitEffYears?: number; // Effective wait time (capped) used for rent calculation
      debug?: {
        doubleCountCheck: {
    ...
    totalCostPerPflopYear: number; // Headline cost (includes scarcity if groundScarcityPricingEnabled=true, used for crossover)
    totalCostPerPflopYearBase?: number; // Base cost (no scarcity pricing)
    totalCostPerPflopYearAllIn?: number; // All-in cost (includes full delay penalty, for reference/debug)
    totalCostPerPflopYearEffective?: number; // Effective cost (always includes scarcity, alias for allIn)
    pricingComponents?: {
      delayPenaltyWeighted: number;
      scarcityRent: number;
      pricingMode: 'none' | 'partial' | 'full';
      delayPenaltyWeight: number;
      scarcityRentWeight: number;
    };
    // Replacement/ops breakdown
    replacementCost?: number;
    sparesCarryCost?: number;
    opsCost?: number;
  ...
  orbit: {
    ...
    totalCostPerPflopYearEffective?: number; // Effective cost (includes all adders, for crossover comparison)
...
export interface YearParams {
  ...
  // Ground constraint pricing mode
  groundConstraintPricingMode?: 'none' | 'partial' | 'full'; // Default 'partial'
  groundDelayPenaltyWeight?: number; // [0,1], default 0.6
  groundScarcityRentWeight?: number; // [0,1], default 0.3
  groundScarcityPricingEnabled?: boolean; // If true, include scarcity adders in headline cost (default true)
  // WACC capital rationing (WACC rises with backlog)
  wacc?: number; // Base WACC (default 0.10)
  waccBacklogK?: number; // Scaling factor for backlog impact (default 0.5)
  waccBacklogExponent?: number; // Exponent for convexity (default 1.2)
  criticalBacklogGW?: number; // Critical backlog threshold (default 50 GW)
  // Scarcity rent parameters (Hill function for saturating wait-time-based rent)
  scarcityRentWaitCapYears?: number; // Maximum wait time cap (default 10)
  scarcityRentWaitThresholdYears?: number; // Threshold for half-saturation (default 1.5)
  scarcityRentMaxFracOfCapexAnnual?: number; // Maximum rent as fraction of annualized capex (default 0.8)
  scarcityRentShapeP?: number; // Hill function shape parameter (default 2.0)
  
  // Ground replacement/ops assumptions
  groundAnnualFailureRate?: number; // Default 0.03
  groundSpareFraction?: number; // Default 0.08
  groundOpsCostPerPflopYear?: number; // Default 0
  groundReplacementMode?: 'none' | 'expected'; // Default 'expected'
