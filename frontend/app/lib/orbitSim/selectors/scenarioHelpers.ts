...
export function debugScenarioData(scenarioMode?: ScenarioMode): void {
  ...
  if (series.length > 0) {
    ...
    console.log(`[DEBUG] Scenario data for ${scenarioKey}:`, {
      ...
      first: {
        year: first.year,
        cost_ground: first.physics_cost_per_pflop_year_ground,
        cost_orbit: first.physics_cost_per_pflop_year_orbit,
        cost_mix: first.physics_cost_per_pflop_year_mix,
        carbon_orbit: first.orbit_carbon_intensity ?? 0,
        orbit_share: first.orbit_compute_share,
      },
      last: {
        year: last.year,
        cost_ground: last.physics_cost_per_pflop_year_ground,
        cost_orbit: last.physics_cost_per_pflop_year_orbit,
        cost_mix: last.physics_cost_per_pflop_year_mix,
